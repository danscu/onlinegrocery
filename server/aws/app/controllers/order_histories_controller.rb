class OrderHistoriesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @order_history.save
        ...
        format.json { render action: 'show', status: :created, location: @order_history }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to order_histories_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def order_history_params
      params.require(:order_history).permit(:itemID, :quantity, :pricePerUnit)
    end
end
